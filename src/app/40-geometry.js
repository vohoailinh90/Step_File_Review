...
function showInfo(data){
  const el = $('info');
  if (!data){ el.classList.remove('show'); el.innerHTML = ''; return; }
  let html = '<div class="hd">' + data.title + '</div>';
  data.rows.forEach(([k,v]) => { html += '<div><span class="k">' + k + '</span> <span class="v">' + v + '</span></div>'; });
  if (data.foot) html += '<div class="k" style="margin-top:5px">' + data.foot + '</div>';
